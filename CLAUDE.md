...
- `bim-lva-composer-ifc.html` — **канонический вьювер**, ~900 КБ, весь код в одном
  файле. Версия и сборка — в объекте `COMPOSER_ABOUT` (сейчас `2026.08.09`).
...
- `sw-composer.js` — service worker; при правках вьювера поднимать `CACHE`
  (сейчас `bimlva-composer-shell-v93`). Вендоры — cache-first, свой JS/CSS —
  network-first.
- `FEATURES.md` — трекер фич вьювера (пункты 1–35 закрыты).
