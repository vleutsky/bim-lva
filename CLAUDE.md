...
- `bim-lva-composer-ifc.html` — **канонический вьювер**, ~900 КБ, весь код в одном
  файле. Версия и сборка — в объекте `COMPOSER_ABOUT` (сейчас `2026.08.05`).
...
- `sw-composer.js` — service worker; при правках вьювера поднимать `CACHE`
  (сейчас `bimlva-composer-shell-v81`). Вендоры — cache-first, свой JS/CSS —
  network-first.
- `FEATURES.md` — трекер фич вьювера (пункты 1–25 закрыты).
...
## 6. Платное и бесплатное на ленте
...
Сейчас бесплатны только `LVA_Version` и `LVA_CatalogDoctor`. **Остальные ~87
