...
npm run test-geo-seq # файлы по одному: абсолютные координаты каждого на месте
...
- Проверено и **не подтвердилось** (не тратить время заново): сводка «метры +
  миллиметры» (`npm run test-units`) и посадка файлов при загрузке **по одному**
  (`npm run test-geo-seq`, сверяются абсолютные координаты, а не разнос центров).
  Ошибка 0.00 м в обоих. Была попытка «чинить» `ifcWorldOrigin` после
  `snapSceneContentToOrigin` — тест показал, что она ничего не меняет, правка
  откачена. Тест, который «воспроизвёл» ошибку на 177 м, врал сам: ожидаемое
  расстояние считалось по точкам привязки фикстур, а не по центрам сеток.
