...
1. `20260806140000_licenses_lic_format.sql` — добавляет `machine_id`, разрешает
   бессрочные лицензии, ограничивает продукты теми, что знает код: `Civil`,
   `Navis`, `Inventor`, `*`;
2. `20260812180000_licenses_user_id_optional.sql` — снимает `NOT NULL` с
   `licenses.user_id`, без неё не работает учёт офлайн-лицензий.

Подробная инструкция с проверками состояния и разбором сообщений —
