...
| `20260806140000_licenses_lic_format.sql` | `machine_id`, бессрочные лицензии, список продуктов | **нужно применить** |
...
- снимает `NOT NULL` с `licenses.user_id`;
- пересоздаёт политику чтения `licenses_select_own` так, чтобы строку без
  `user_id` видел только админ. По смыслу поведение то же, что и было: в SQL
  сравнение `null = uuid` даёт `null`, а не `true`, поэтому чужую строку
  пользователь и раньше не видел. Теперь это написано в коде явно, а не следует
...
`((user_id IS NOT NULL) AND (user_id = auth.uid())) OR is_license_admin()`.
...
Обе миграции прогнаны на живом PostgreSQL 16 в том же порядке, что выше, со
