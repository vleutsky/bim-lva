/**
 * Повёрнутая вставка + тесселированная геометрия — случай с объекта, доведённый
 * до подтверждённого диагноза и правки во вьювере.
 *
 ...
 * геометрия лежит в `IfcPolygonalFaceSet` с ЛОКАЛЬНЫМИ вершинами. «Диагностика
 * IFC» → «Цепочка размещения» на реальном файле подтвердила: IfcSite и элемент
 * стоят на identity-размещении, а этот сдвиг лежит в
 * IfcGeometricRepresentationContext.WorldCoordinateSystem — сценарий
 * `worldInContext: true` ниже воспроизводит его буквально.
 *
 * Случай `worldInContext: true` — регрессия для фикса в
