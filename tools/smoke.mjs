...
async function checkRoadCrossSections(page) {
    ...
    /*
     * Автопостроение узлов здесь отключаем НАМЕРЕННО. Проверки ниже про
     * поперечники и коридоры, а оси-фикстуры лежат в паре метров друг от
     * друга и пересекаются: узел обрезал бы коридор у устья, и станция на
     * PI просто исчезала бы из выборки. Сами узлы закрыты отдельным тестом
     * (`npm run test-intersections`), включая и автопостроение.
     */
    await page.evaluate(() => window.BimLvaDebug.setAutoNodes(false));
    ...
    if (Math.abs(slopes.fill - 1.125) > 0.08) {
        // Разбивка по сторонам: суммой не видно, обе стороны плохи или одна.
        problems.push(`поперечники: объём откосов от кромок ${slopes.fill} вместо ≈1.125`
            + ` (по сторонам ${JSON.stringify(slopes.res?.sides)})`);
    }
