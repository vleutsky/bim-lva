...
25. [x] **Лицензии** — заявка в кабинете, выпуск ключей Edge Function, офлайн-проверка Ed25519
...
- `npm run test-license` — проверка формата лицензионного ключа: подпись,
  подделка, просрочка. Использует тот же модуль, что и Edge Function, и сверяет
  с ним однофайловую сборку.
