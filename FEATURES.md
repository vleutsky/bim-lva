...
- [ ] **NWC / NWD / NWF (Navisworks)** — в браузере не читаются (проприетарный формат, нет open-source WASM-парсера). Варианты позже:
  1. Пользователь: Navisworks → Export IFC / Publish, затем открыть IFC в Composer
  2. Облачный конвертер (ODA BimNv или свой Navisworks API на сервере) → IFC / glTF → Composer
  3. Не путать с «кэшем»: `.nwc` сам по себе не предназначен для обмена; для шаринга обычно `.nwd`
